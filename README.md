...
- **Geo split tunneling (国内直连)**: mainland-China routes (5,493 aggregated CIDRs from APNIC delegation data) are pinned to the physical underlay while everything else traverses the tunnel — domestic sites stay direct and fast, foreign traffic goes through the VPN. Linux CLI: `-cn-direct` with `-take-route`; Windows GUI: automatic in split-tunnel mode; Android: `splitTunnel` now installs the generated non-CN whitelist (`split_routes_geo.xml`, 12,014 routes). Refresh the lists with `bash scripts/update-chnroute.py`.
...
- **Opt-in probe mark** (`probe_mark` in server/client JSON): embeds a visible deployment-identifying tag at the head of every handshake cover, making **that deployment only** identifiable on the wire. Two narrow uses: measurement research (deploy tagged servers, observe which tags the censor acts on — the methodology of GFW mapping papers) and politeness (a blockable variant so censors can target the tool without collateral over-blocking). Off by default; never enable it on a server whose users need undetectability.
